...
Translate the user's constraints into flags – every one of them, on this read. Re-supply still-active constraints on each refinement instead of assuming a previous filtered read changed the underlying search. Use `--stops 0` for nonstop and `--stops 0,1` for at most one stop – the flag selects a set, never a ceiling (see below).
