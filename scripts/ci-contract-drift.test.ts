 ...
 *
 * Three claims, each a line someone could plausibly "tidy up":
 *
 ...
 *     cadence. Drop `continue-on-error` or let one branch skip `exit 0` and
 *     every CLI pull request after an unrelated API release is blocked on a
 *     contract its author never touched. Nothing else would notice: the step
 *     still passes on the day it is changed.
 *   - The contract is fetched from production only. The document carries a
 *     `servers` block naming its host, so a staging URL would rewrite that line
 *     on every refresh and compare against a contract nothing ships against.
 *   - `src/api-types.d.ts` is generated by the `typecheck` script, not
 *     committed. Checks A and C are compile-time comparisons against it; a
 *     committed copy is a second contract to keep in step. If generation were a
 *     workflow step, only that lane would be covered: the release lane and
 *     developers' terminals would compare against whatever `postinstall` last
 *     left, or nothing.
 */
...
/** The other lane that typechecks, which is why generation belongs to the script. */
...
describe("ci-cli: the contract drift step cannot veto a merge", () => {
  ...
  it("ends every branch of its script in `exit 0`", () => {
    // `continue-on-error` survives a rewrite of the script; `exit 0` on every
    // path survives a rewrite of the step. Together, one edit cannot make this
    // a gate.
  ...
  it("warns rather than fails, and says how to fix it", () => {
    ...
    // A failed fetch is a notice, not a warning: a flaky CDN is not drift, and
    // unactionable warnings train people to ignore the real one.
...
describe("the generated types exist before anything compares them", () => {
  // Generation lives in the `typecheck` script so every caller inherits it. A
  // workflow step would cover one lane only, and without the file Checks A and
  // C resolve to `never` and report errors about code that is fine.
  it("are regenerated by the typecheck script itself", () => {
    const typecheck = scripts.typecheck ?? "";
    // Anchored and positive across the whole generate-to-bunx boundary, since a
    // blacklist only refuses separators someone thought of: `;bunx`, a bare `&`
    // (tsc racing a backgrounded generate), a newline, or `||` (tsc only when
    // generation failed) would all slip past a `; bunx` check.
  ...
  it("are not regenerated by a second, drifting copy in ci-cli", () => {
    // A standalone step here would be a second copy to keep in step with
    // package.json, and this lane would silently diverge from the others.
...
describe("package.json: the contract scripts", () => {
  // The typecheck and refresh assertions only check that another script
  // mentions `api-types:generate`, not what it does. Pointed at `echo noop`,
  // both stay green while Checks A and C compare against stale types.
  ...
  // during `bun install`, which runs beside the signing identity and the store
  // token in three lanes. Empty means no dependency executes on install. The
  // root package's own `postinstall` is unaffected.
  ...
  it("regenerates the types as part of the refresh, and only on success", () => {
    ...
    // stopped at the JSON would leave them stale for Checks A and C.
    //
...
describe("the vendored contract is committed", () => {
  ...
  it("is what the generator reads", () => {
    ...
    // Monorepo paths and `bun run --filter cli` do not exist in this repo.
  ...
  it("is typechecked locally even when it is the only staged file", () => {
    // A refresh commit is `contract/openapi.json` alone (CONTRIBUTING says so,
    // and `src/api-types.d.ts` is gitignored), so no `*.ts` is staged. A stanza
    // keyed on `*.ts` only would skip the typecheck on exactly that commit.
    //
  ...
  it("is formatted by biome, like every other committed file", () => {
    // The refresh runs biome so its output matches `bun run lint`, rather than
    // exempting the file: an exemption is invisible from the file itself, and
    // this repository has one formatter.
