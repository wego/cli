 ...
 *
 * The CLI is a separate repository from the API, so it has no path to read the
 * contract from at build time. It vendors one instead: this script fetches the
 * published document and writes it here, and `api-types:generate`, Checks A and
 * C, and Check B run against the committed copy. To refresh: run this, commit
 * the JSON diff on its own, then fix whatever the checks report.
 *
 ...
 * contract. Without it the next `bun run typecheck` would compare the CLI
 * against shapes the API no longer publishes.
 *
 ...
 * refresh and make the diff unreadable, and would vendor a contract nothing
 * ships against.
 */
 ...
 *  connection that opens and then stalls would hang forever and never reach the
 *  retry. 15s is generous for a ~200KB document; the CI drift step bounds its
 *  own fetch the same way with `curl --max-time 20`. */
...
/** A first pass at the file. Biome owns the final shape (`formatContract`);
 *  this keeps the file on disk sane if the format step fails. */
export function serializeContract(document: ContractDocument): string {
  return `${JSON.stringify(document, null, 2)}\n`;
}

/**
 * The contract is committed, so `bun run lint` checks it like every other file.
 * Formatting it here, rather than exempting `contract/`, keeps a refresh from
 * leaving the tree lint-red. Biome's output is deterministic, so fetching an
