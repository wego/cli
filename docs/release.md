...
| `integration (<target>)` | one per target | The integration tier (`integration/`) against the binary `build` produced for that target, on that target's own runner: linux x64 and arm64, macOS arm64 and Intel. `release` **needs** all four, so a binary that fails its scenarios is never published. **Windows is built and published without a leg**: its first run (v1.4.0) failed on the suite, not the binary, and held the release. There is no override, by design: a flaky scenario or a stuck runner holds `cli/next` until **Re-run failed jobs** on the run passes it, and a real failure is fixed and re-tagged |
...
| Mint the plugin publish token | A plugin key GitHub refuses (a wrong or deleted `SKILLS_PUBLISH_APP_PRIVATE_KEY`), or an App that cannot reach `wego/skills`. Minted before the move so a bad key costs a re-run, not a rollback |
